# SHN shinerium copy of Beacon

defmodule Explorer.Chain.Shinerium.Reader do
  @moduledoc "Contains read functions for shinerium chain related modules."
  ...
  alias Explorer.Chain.Shinerium.{Blob, BlobTransaction}

  @doc """
  Finds `t:Explorer.Chain.Shinerium.Blob.t/0` by its `hash`.

  Returns `{:ok, %Explorer.Chain.Shinerium.Blob{}}` if found

      iex> %Explorer.Chain.Shinerium.Blob{hash: hash} = insert(:blob)
      iex> {:ok, %Explorer.Chain.Shinerium.Blob{hash: found_hash}} = Explorer.Chain.Shinerium.Reader.blob(hash, true)
      iex> found_hash == hash
      true
  ...
  Returns `{:error, :not_found}` if not found
      ...
      ...> )
      iex> Explorer.Chain.Shinerium.Reader.blob(hash, true)
      {:error, :not_found}
  ...
  @doc """
  Finds all `t:Explorer.Chain.Shinerium.Blob.t/0`s for `t:Explorer.Chain.Transaction.t/0`.

  Returns a list of `%Explorer.Chain.Shinerium.Blob{}` belonging to the given `transaction_hash`.

      iex> blob = insert(:blob)
      iex> %Explorer.Chain.Shinerium.BlobTransaction{hash: transaction_hash} = insert(:blob_transaction, blob_versioned_hashes: [blob.hash])
      iex> blobs = Explorer.Chain.Shinerium.Reader.transaction_to_blobs(transaction_hash)
  ...
  Returns a list of `%{block_consensus: boolean(), transaction_hash: Hash.Full.t()}` maps for all found transactions.
      ...
      iex> %Explorer.Chain.Shinerium.Blob{hash: blob_hash} = insert(:blob)
      iex> %Explorer.Chain.Shinerium.BlobTransaction{hash: transaction_hash} = insert(:blob_transaction, blob_versioned_hashes: [blob_hash])
      iex> blob_transactions = Explorer.Chain.Shinerium.Reader.blob_hash_to_transactions(blob_hash)
